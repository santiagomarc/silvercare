...
class _BloodPressureScreenState extends State<BloodPressureScreen> {
 ...
 Future<void> _saveBloodPressureData() async {
  ...
  try {
    ...
    // 1️⃣ Create a temporary BP entry for the UI
    final newBP = HealthDataModel(
      id: '', // Firestore ID not yet available
      elderlyId: 'tempUser', // replace with _auth.currentUser!.uid
      type: 'blood_pressure',
      systolic: systolic,
      diastolic: diastolic,
    ...
    setState(() {
      _bpData.insert(0, newBP); // show instantly
    });
  ...
  List<Widget> _buildBPRecords(BuildContext context) {
    ...
    return _bpData.take(10).map((data) {
      final sys = data.systolic?.toInt() ?? 0;
      final dia = data.diastolic?.toInt() ?? 0;
