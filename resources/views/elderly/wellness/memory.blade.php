<x-dashboard-layout>
    ...
    <div x-data="memoryGame()">
        <x-dashboard-nav
            title="Mind Games"
            subtitle="Challenge your memory with cards"
            role="elderly"
        ...
        function memoryGame() {
            return {
                ...
                iconLibrary: {
                    ...
                    paw: { color: '#a855f7', path: '<path d="M12 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm-4 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm8 0c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zM6 9c-1.66 0-3 1.34-3 3 0 1.66 1.34 3 3 3 .2 0 .38-.04.56-.09.68 1.23 2.37 2.09 4.44 2.09s3.76-.86 4.44-2.09c.18.05.36.09.56.09 1.66 0 3-1.34 3-3 0-1.66-1.34-3-3-3-1.66 0-3 1.34-3 3 0 .21.04.41.09.61C15.41 9.41 13.8 9 12 9s-3.41.41-4.09 1.61C7.96 10.41 8 10.21 8 9c0-1.66-1.34-3-2-3z"/>' },
                    fish: { color: '#f97316', path: '<path d="M21.5,12c0,0-2.5,2-5.5,2s-4-2-4-2s1-2,4-2S21.5,12,21.5,12z M3,12l2-2h2c0,0,3-2,6-2c3,0,5,2,5,2l2,2l-2,2c0,0-2,2-5,2 c-3,0-6-2-6-2H5L3,12z M15,12c0-0.6-0.4-1-1-1s-1,0.4-1,1s0.4,1,1,1S15,12.6,15,12z"/>' },
                    bug: { color: '#16a34a', path: '<path d="M12,2C9,2,6,4,6,7v2H4v2h2v2H4v2h2v1c0,3,2,6,6,6s6-3,6-6v-1h2v-2h-2v-2h2V9h-2V7C18,4,15,2,12,2z M12,4c2,0,4,1,4,3v2 h-3V6h-2v3H8V7C8,5,10,4,12,4z M8,16H11v3H11C9,19,8,18,8,16z M16,16c0,2-1,3-3,3v-3H16z"/>' },
                    bird: { color: '#0ea5e9', path: '<path d="M21,6c-1,1-2,2-4,2c0-3-3-5-6-5C7,3,4,6,4,10c0,2,1,4,2,5l-4,4l2,2l4-4c1,1,3,2,5,2c4,0,7-3,7-7c0-2-1-4-3-5 c2,0,3-1,4-2L21,6z"/>' },
                    ...
                    dog: { color: '#854d0e', path: '<path d="M12,2C9,2,6,4,6,7v3c-2,1-3,3-3,6c0,3,2,6,5,6h8c3,0,5-3,5-6c0-3-1-5-3-6V7C18,4,15,2,12,2z M8,14c-1,0-2-1-2-2 s1-2,2-2s2,1,2,2S9,14,8,14z M16,14c-1,0-2-1-2-2s1-2,2-2s2,1,2,2S17,14,16,14z"/>' }
                },
                levelSets: [
                    ['leaf', 'tree', 'flower', 'sun', 'cloud', 'mountain'],
                    ['apple', 'pizza', 'icecream', 'coffee', 'burger', 'carrot'],
                    ['car', 'plane', 'boat', 'bus', 'bike', 'map'],
                    ['star', 'heart', 'diamond', 'bell', 'key', 'lock'],
                    ['paw', 'fish', 'bug', 'bird', 'cat', 'dog']
                ],

                init() { this.startLevel(); },
                restartLevel() { this.startLevel(); },
                restartGame() { this.level = 0; this.startLevel(); },
                ...
                checkForMatch() {
                    ...
                    if(this.cards[i1].iconName === this.cards[i2].iconName) {
                        ...
                        if(this.cards.every(c => c.matched)) {
                            setTimeout(() => {
                                if (this.level === 4) this.showGrandModal = true;
                                else this.showLevelModal = true;
                            }, 600);
                        }
                    } else {
                        setTimeout(() => {
