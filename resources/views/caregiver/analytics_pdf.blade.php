...
<body>
    ...
    <div class="header">
        ...
        <div class="report-meta">
            ...
            <p style="font-size:10px;color:#666;margin:4px 0 0;">
                Data current as of {{ ($reportGeneratedAt ?? now())->format('M j, Y \a\t g:i A') }}
            </p>
    ...
    {{-- M3: Dose timing. Adherence percentage alone hides a patient who takes
         every dose but consistently hours late — clinically different from one
         who takes them on time. --}}
    @if(($doseLateness ?? collect())->isNotEmpty())
    <div class="section">
        <h3 class="section-title">Dose Timing (last 7 days)</h3>
        <table class="data-table">
            <thead>
                <tr>
                    <th>Medication</th>
                    <th>Doses taken late</th>
                    <th>Average delay</th>
                </tr>
            </thead>
            <tbody>
                @foreach($doseLateness as $row)
                <tr>
                    <td>{{ $row['name'] }}</td>
                    <td>{{ $row['late_count'] }}</td>
                    <td>{{ $row['average_minutes_late'] }} min</td>
                </tr>
                @endforeach
            </tbody>
        </table>
    ...
    {{-- M3: Alert history. Without this the report shows current state but not
         what happened during the week it covers. --}}
    @if(($alertHistory ?? collect())->isNotEmpty())
    <div class="section">
        <h3 class="section-title">Alert History (last 7 days)</h3>
        <table class="data-table">
            <thead>
                <tr>
                    <th>When</th>
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $alert->created_at->format('M j, g:i A') }}</td>
                    <td>
                        <span class="badge {{ in_array($alert->severity, ['critical','emergency']) ? 'badge-red' : ($alert->severity === 'warning' ? 'badge-yellow' : 'badge-green') }}">
                            {{ strtoupper($alert->severity) }}
                        </span>
                    </td>
                    <td>{{ $alert->title }}</td>
                    <td>
                        {{ ucfirst($alert->state) }}@if($alert->acknowledged_at)
                            ({{ $alert->created_at->diffInMinutes($alert->acknowledged_at) }} min to acknowledge)
                        @endif
                    </td>
                </tr>
                @endforeach
    ...
    {{-- M3: Provenance. A clinician needs to know which readings the patient
         entered by hand and which came from a connected device. --}}
    @if(($sourceAttribution ?? collect())->isNotEmpty())
    <div class="section">
        <h3 class="section-title">Where This Data Came From (last 7 days)</h3>
        <table class="data-table">
            <thead>
                <tr><th>Source</th><th>Readings</th></tr>
            </thead>
            <tbody>
                @foreach($sourceAttribution as $source => $count)
                <tr>
                    <td>{{ \Illuminate\Support\Str::headline(str_replace('_', ' ', $source)) }}</td>
                    <td>{{ $count }}</td>
                </tr>
                @endforeach
            </tbody>
    ...
    <!-- Footer -->
    <div class="footer">
        {{-- M3: freshness stamp. A printed report with no date is indistinguishable
             from a current one no matter how old it gets. --}}
        <p><strong>Data current as of {{ ($reportGeneratedAt ?? now())->format('M j, Y \a\t g:i A') }}</strong>
           ({{ $elderly->timezone ?? config('app.timezone') }})</p>
        <p>This report was generated by SilverCare Health Monitoring System.
           Figures are computed from recorded data and are not a medical assessment.</p>
